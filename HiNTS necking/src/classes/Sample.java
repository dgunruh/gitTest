...
public class Sample {
    ...
    public Sample(Map<String,Object> params){
    	...
    	if(feature=="mobility"){
    		//voltage = 30*Constants.kelvintory*0.1*25 / Constants.sqrt2; //25 nanoparticles, so each get this 30K voltage
    		voltage = 30*Constants.kelvintory*0.1*20 / Constants.sqrt2;
		...
		//System.out.println(nanoparticles[10]);
    ...
    private void loadConfiguration()  {
    	...
    	if(!bimodal){
    		...
    		/*String prefix= "./data/";
    		//String middle = String.valueOf(nnanops)+"_"+Configuration.diameter;
    		String middle = "QD_com_xyz_1846_trimmed_square3";
    		if(sample_number == 1) {
    	...
    	)
    	{
    	   ...
    	   nnanops = count.getLineNumber() - 4; // +1 because line index starts at 0, -6 for starting lines, -1 because ends in empty line
    ...
    public void get_necked_neighbors() {
		...
		//String neck_filename = "./data/QD_necking_1846_trimmed_square.txt";
		...
		for(int i=0; i<nanoparticles.length; i++){
			...
			if(!thisNeckNeighborList.isEmpty()) {
				for(int j=0; j<thisNeckNeighborList.size()/2; j++){
					...
					//nearestNeighbors.remove(nanoparticles[k]); //no hopping transport when necks exist
					total_neckedNeighbors += 1;
				}
				
				//System.out.println("Nanoparticle id is: " + nanoparticles[i].id + " and the number of necked neighbors is: " + total_neckedNeighbors);
			}
