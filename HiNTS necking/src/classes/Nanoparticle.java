...
public class Nanoparticle {
	...
	public Nanoparticle(double xcoord, double ycoord, double zcoord, double diameter_input, int id_value, Sample sample) {
		...
		double x0 = 3.2*Constants.nmtobohr;
		...
		switch(sample.latticeStructure){
			case "triclinic":
				double m_edge = 1/Math.tan(sample.latticeAngleGamma); //equation is z-z0 = m*(x-x0)
				...
				//calculate which layer of the sample the NP lives in
				double y_layer = (y-(Configuration.latticeBasisLength*Constants.nmtobohr/2))/(y_3*Constants.nmtobohr);
				...
				double z_left = m_edge*(x+x_mod) + z_mod + x0/2;
				double z_right = sample.cellz + m_edge*(x-x_mod) + z_mod -x0/2;
